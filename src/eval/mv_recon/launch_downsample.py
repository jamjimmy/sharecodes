...
def get_args_parser():
    ...
    parser.add_argument(
        ...
        help="max points kept for point-cloud evaluation; <=0 disables",
    )
    parser.add_argument(
        "--pcd_voxel_size",
        type=float,
        default=0.001,
        help="voxel size for optional Open3D downsampling; <=0 disables",
    )
    return parser


def subsample_corresponded_points(pred_pts, gt_pts, colors, max_points):
    if max_points is None or max_points <= 0:
        return pred_pts, gt_pts, colors
    n = pred_pts.shape[0]
    if n <= max_points:
        return pred_pts, gt_pts, colors
    idx = np.random.choice(n, size=max_points, replace=False)
    return pred_pts[idx], gt_pts[idx], colors[idx]
            ...
            with accelerator.split_between_processes(list(range(len(dataset)))) as idxs:
                for data_idx in tqdm(idxs):
                    ...
                    if args.pcd_voxel_size > 0:
                        pcd = pcd.voxel_down_sample(args.pcd_voxel_size)
                        pcd_gt = pcd_gt.voxel_down_sample(args.pcd_voxel_size)
                    # o3d.io.write_point_cloud(
