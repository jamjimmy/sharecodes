        ...
        for name_data, dataset in datasets_all.items():
            ...
            with accelerator.split_between_processes(list(range(len(dataset)))) as idxs:
                for data_idx in tqdm(idxs):
                    if data_idx < args.start_idx and args.start_idx >=0:
                        continue
                    ...
                    if model_name == "stream3r" or "VGGT":
                        ...
                        with torch.cuda.amp.autocast(dtype=dtype):
                            ...
                            os.makedirs(os.path.join(save_path,"perf_logs"), exist_ok=True)
                            with open(os.path.join(save_path,"perf_logs",f"{str(data_idx)}.json"), "w", encoding="utf-8") as f:
