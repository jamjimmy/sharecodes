...
def get_args_parser():
    ...
    parser.add_argument(
        "--conf_thresh", type=float, default=0.1, help="confidence threshold"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument("--loop_closure_neighbor_k", type=int, default=21)
    parser.add_argument(
        "--nrgbd_frame_ids",
        type=str,
        default="",
        help='NRGBD 仅重建给定图像 id（与 img{N}.png / depth{N}.png 的 N 一致），逗号分隔，例如 "20,25,60"。留空则仍按 kf_every/max_frames 采样。',
    )
    return parser


def _colors_for_o3d(rgb: np.ndarray) -> np.ndarray:
    """数据集 ImgNorm 常为 [-1,1]，Open3D 顶点色需要 [0,1]。"""
    c = np.asarray(rgb, dtype=np.float64).reshape(-1, 3)
    if c.size == 0:
        return c
    if c.min() < -1e-3:
        c = (np.clip(c, -1.0, 1.0) + 1.0) * 0.5
...
def main(args):
    ...
    #     conf_threshold = 0.0
    # else:
    #     conf_threshold = np.percentile(conf, args.conf_thresh)
    ...
    nrgbd_frame_ids = None
    if args.nrgbd_frame_ids.strip():
        nrgbd_frame_ids = [
            int(x.strip()) for x in args.nrgbd_frame_ids.split(",") if x.strip()
        ]

    datasets_all = {
        # "7scenes": SevenScenes(
        #     split="test",
        #     ROOT="/data/jiangzj/InfiniteVGGT/data/7-scenes/processed",
        #     resolution=resolution,
        #     num_seq=1,
        #     full_video=True,
        #     kf_every=2,
        #     max_frames=args.max_frames,
        #     frame_ids=nrgbd_frame_ids,
        # ),  # 20),
        "NRGBD": NRGBD(
            split="test",
            ROOT="/data/jiangzj/InfiniteVGGT/data/neural_rgbd_data",
            resolution=resolution,
            num_seq=1,
            full_video=True,
            kf_every=2,
        ...
        for name_data, dataset in datasets_all.items():
            ...
            with accelerator.split_between_processes(list(range(len(dataset)))) as idxs:
                for data_idx in tqdm(idxs):
                    ...
                    m_pred = np.isfinite(pts_all).all(axis=-1)
                    m_gt = np.isfinite(pts_gt_all).all(axis=-1)
                    depth_ok = masks_all.astype(bool)

                    # 论文插图：仅用「深度有效 ∩ 坐标有限」的 GT，颜色与逐像素对齐
                    gt_paper_valid = depth_ok & m_gt
                    pts_paper = pts_gt_all[gt_paper_valid].reshape(-1, 3)
                    cols_paper = _colors_for_o3d(images_all[gt_paper_valid])
                    if pts_paper.shape[0] > 0:
                        pcd_paper = o3d.geometry.PointCloud()
                        pcd_paper.points = o3d.utility.Vector3dVector(pts_paper)
                        pcd_paper.colors = o3d.utility.Vector3dVector(cols_paper)
                        o3d.io.write_point_cloud(
                            osp.join(
                                save_path,
                                f"{scene_id.replace('/', '_')}-gt_paper.ply",
                            ),
                            pcd_paper,
                        )

                    # 与预测对齐 / ICP：预测与 GT、深度在同一套像素上
                    align = m_pred & m_gt & depth_ok
                    pts_all_masked = pts_all.reshape(-1, 3)
                    pts_gt_all_masked = pts_gt_all.reshape(-1, 3)
                    images_all_masked = images_all.reshape(-1, 3)
                    ...
                    if args.use_proj:
                        ...
                        pts_all_masked = (s * (R @ pts_all_masked.T)).T + t

                    o3d_cols = _colors_for_o3d(images_all_masked)

                    pcd = o3d.geometry.PointCloud()
                    pcd.points = o3d.utility.Vector3dVector(
                        pts_gt_all_masked.reshape(-1, 3)
                    )
                    pcd.colors = o3d.utility.Vector3dVector(o3d_cols)
                    o3d.io.write_point_cloud(
                        os.path.join(
                    ...
                    pcd_gt.colors = o3d.utility.Vector3dVector(o3d_cols)
                    # o3d.io.write_point_cloud(
