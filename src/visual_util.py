...
def predictions_to_glb(
    ...
    Args:
        ...
        color_mode (str): Point color mode, "RGB" or "Confidence"
    ...
    if color_mode == "Confidence":
        if filtered_conf.size > 0:
            conf_min = float(filtered_conf.min())
            conf_max = float(filtered_conf.max())
            if conf_max > conf_min:
                conf_norm = (filtered_conf - conf_min) / (conf_max - conf_min)
            else:
                conf_norm = np.ones_like(filtered_conf)
            confidence_colormap = matplotlib.colormaps.get_cmap("turbo")
            colors_rgb = (confidence_colormap(conf_norm)[:, :3] * 255).astype(np.uint8)
        else:
