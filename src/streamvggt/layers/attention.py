...
class Attention(nn.Module):
    ...
    ) -> Union[torch.Tensor, Tuple[torch.Tensor, Tuple], Tuple[torch.Tensor, Tuple, Dict[str, torch.Tensor]]]:
        B, N, C = x.shape
        ...
        if self.fused_attn:
            ...
            if return_attn_stats:
                with torch.no_grad():
                    ...
                    attn_key_mean = attn_probs.mean(dim=(0, 1, 2)).detach()
                    attn_stats = {"attn_key_mean": attn_key_mean}
            ...
            if return_attn_stats:
                with torch.no_grad():
                    attn_key_mean = attn.mean(dim=(0, 1, 2)).detach()
                    attn_stats = {"attn_key_mean": attn_key_mean}
