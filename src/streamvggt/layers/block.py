...
class Block(nn.Module):
    ...
    def forward(
    ...
    ) -> Union[Tensor, Tuple[Tensor, Dict], Tuple[Tensor, Dict, Dict[str, Tensor]]]:
        ...
        def attn_residual_func(
            ...
            if use_cache:
                if return_attn_stats:
                    output, new_kv, attn_stats = self.attn(
                        self.norm1(x),
                        pos=pos,
                    ...
                    return self.ls1(output), new_kv, attn_stats
                output, new_kv = self.attn(
                    self.norm1(x),
                    pos=pos,
                    past_key_values=past_key_values,
                    use_cache=True,
        ...
        if use_cache:
            if return_attn_stats:
                attn_output, new_kv, attn_stats = attn_residual_func(
                    x,
                    pos=pos,
                    past_key_values=past_key_values,
                    use_cache=True,
                ...
                attn_output, new_kv = attn_residual_func(
                    x,
                    pos=pos,
                    past_key_values=past_key_values,
                    use_cache=True,
