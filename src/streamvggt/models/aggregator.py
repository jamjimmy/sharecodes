...
class Aggregator(nn.Module):
    ...
    def forward(
            ...
            for attn_type in self.aa_order:
                if attn_type == "frame":
                    ...
                    if use_cache:
                        ...
                        tokens, global_idx, global_intermediates, new_kv, attn_stats = self._process_global_attention(
                        ...
                        if return_attn_stats and attn_stats is not None:
                            attn_key_stats_list.append(attn_stats["attn_key_mean"])
        ...
        if scores and (not return_attn_stats): # update scores
        ...
        if use_cache:
            if return_attn_stats:
                return output_list, self.patch_start_idx, new_kv_list, attn_key_stats_list
            return output_list, self.patch_start_idx, new_kv_list
    ...
    def _process_global_attention(
        ...
        Tuple[torch.Tensor, int, List[torch.Tensor], List, Optional[Dict[str, torch.Tensor]]],
    ]:
        """
        ...
        for _ in range(self.aa_block_size):
            ...
            if use_cache:
                if return_attn_stats:
                    tokens, new_kv, scores = self.global_blocks[global_idx](
                        tokens,
                        pos=pos,
                        attn_mask=attn_mask,
                    ...
                    tokens, new_kv = self.global_blocks[global_idx](
                        tokens,
                        pos=pos,
                        attn_mask=attn_mask,
                        past_key_values=past_key_values_block,
            ...
            #     del attn_mask
        if use_cache:
            return tokens, global_idx, intermediates, new_kv, scores
